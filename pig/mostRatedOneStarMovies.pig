...
averageRatings = FOREACH ratingsByMovie GENERATE GROUP 
                    AS movieID, AVG(ratings.rating) AS avgRating, COUNT(ratings.rating) AS numRatings;

badMovies = FILTER averageRatings BY avgRating < 2.0;

namesBadMovies = JOIN badMovies BY movieID, nameLookup BY movieID;

finalResults = FOREACH namesBadMovies GENERATE nameLookup::movieTitle AS movieName,
